...
- **Survived:** Variable binària, indicant si el passatger ha sobreviscut (1) o no (0). 

- **Pclass:** En quina classe viatjava el passatger (1, 2 o 3).

- **Name:** Nom del passatger. El format és: Cognom + coma + títol + nom + sobrenom en parèntesis. Aquesta estructura fixa es pot utilitzar per subdividir aquesta variable en Cognom, Títol, Nom.
...
La mediana de l'edat dels passatgers era `r median(dades$Age, na.rm=T)` i la mitjana `r round(mean(dades$Age, na.rm=T),3)`. També veiem que les edats més freqüents són entre els 20 i 30 anys.
...
La variable Name la reorganitzem en FamilyName, Title i Name. Title i FamilyName podrien ser útils per reconstruir l'edat dels passatgers i relacions de parentiu.
...
També veiem d'atres títols que corresponen a un rang militar (Capt, Col, Major), títols nobiliaris (Sir, Lady, the Countess, Jonkheer - títol nobiliari dels Països Baixos). Tots ells denoten un status social elevat, i en ser pocs, els podem agrupar en una mateixa categoria (Noble). També hi afegirem Dr, que només en tenim 8, i en aquella època també denatava un estatus social elevat.
...
Ens trobem amb `r nrow(df_passengers_cabin)` casos, que impliquen les cobertes de la B a la F.
...
Comprovem ara que cap d'aquests passatgers té FamSize = 0:
...
En general, veiem que hi ha 713 números de tiquet que corresponen a una sola perona, mentre que a l'altre extrem, hi ha un tiquet que està associat a 11 persones (encaixa amb la mida de la unitat familiar més gran que viatjava al Titànic).
...
Per la gran majoria de persones, el FamSize coincideix amb el número de tiquet. Per contra, en tenim uns 300 pels quals no coincideix.
...
Com a conclusió, la variable Ticket té potencial per afinar la variable FamSize i possiblement per completar la variable cabin. Tanmateix la relació no és clara, i entrar-hi en detall podria ser l'objecte d'un estudi a part.



## Fare

Aquesta variable ens indica quant va costar cada ticket.
...
Observem que aquells registres amb un mateix Ticket, també tenen tots el mateix Fare. Així és d'esperar, ja que Fare correspon al total pagat per cada tiquet (encara que inclogui múltiples persones).

```{r}
dades %>% group_by(Ticket) %>% summarise(nFare = n_distinct(Fare)) %>% arrange(desc(nFare))
```

Tenim només una excepció, el ticket 7534. Però en ser un únic cas entre els 929 tickets que hi ha a les dades, ho considerarem com un error. Per arreglar-ho, recalculem Fare com la mitjana dels dos valors:
...
Es una variable amb un percentatge de dades perdudes molt elevat `r round(sum(is.na(dades$Deck))/nrow(dades)*100,1)`. Segurament es podria fer una imputació a partir de diverses variables, com FarePers, Ticket, Embarked, Pclass. Però és una tasca complexa, fora dels objectius d'aquesta pràctica. De moment, doncs, exclouré aquesta variable de l'anàlisi.
...
## Variables qualitatives: Test chi-quadrat
...
Per un nivell de confiança del 95% podem rebutjar la hipòtesi nul·la i concloem que Sex i Survival no són independents.
...
Finalment, repetirem el procediment del test chi-quadrat per determinar la relació entre la mida de la unitat familiar i el fet de sobreviure:
...
De nou, per un nivell de confiança del 95% podem rebutjar la hipòtesi nul·la i concloem que les dues variables no són independents.
...
Els resultats mostren que les mides familiars més contribueixen a la correlació són aquelles entre 0 i 3. Tenir entre 1 i 3 acompanyants està correlacionat amb sobreviure, mentre viatjar sol, apareix correlacionat amb no sobreviure.
...
Finalment, aquestes variables s'han utilitzat per constuir un model de regressió logística per predir la variable Survived. Una validació "5-fold cross validation" suggereix que el model té una capacitat predictiva del 81%.
...
També, les variables Deck i Cabin tenen molt de potencial per explicara la supervivència al Titànic, ja que estan relacionades amb l'accés a les sortides cap als bots salvavides. Un estudi per imputar aquestes variables podria ser molt útil. Finalment, la variable FarePers s'ha tractat com a variable contínua, i contenia algun valor extrem. Una discretització d'aquesta variable podria donar millors resultats en la capacitat predictiva del model.
...
[1] RMS Titanic, Wikipecia. https://ca.wikipedia.org/wiki/RMS_Titanic (darrer accés: 07/06/2020)


[2] Encyclopedia Titanica (2004) Cabin Allocations (ref: #3216, published 18 July 2004, generated 22nd May 2020 09:54:03 AM); URL : https://www.encyclopedia-titanica.org/cabins.html (darrer accés: 07/06/2020)
